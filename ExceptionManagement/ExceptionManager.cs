...
namespace ExceptionManagement
{
	...
	public sealed class DefaultPublisher : IExceptionPublisher
	{
		...
		public void Publish(Exception exception, NameValueCollection additionalInfo, NameValueCollection configSettings)
		{
            ...
            StringBuilder strInfoDB = new StringBuilder();
			...
			if (additionalInfo != null)
			{
                ...
                strInfoDB.Append("****** Additional Info *******");

				foreach (string i in additionalInfo)
				{
					strInfo.AppendFormat("{0}{1}: {2}", Environment.NewLine, i, additionalInfo.Get(i));
                    strInfoDB.Append("   [" + i + ":" + additionalInfo.Get(i) + "]");
				}
			}
			#endregion
			...
			if (exception == null)
			{
                ...
                strInfoDB.Append("   [No Exception object has been provided.]");
			}
			else
			{
			#region Loop through each exception class in the chain of exception objects
				...
				do
				{
                    ...
                    strInfoDB.Append("   ****** " + intExceptionCount.ToString() + " Exception Information ******");
                    strInfoDB.Append("   [Exception Type: " + currentException.GetType().FullName + "]");
						...
						if (p.Name != "InnerException" && p.Name != "StackTrace")
						{
							...
							else
							{
								...
								if (p.Name == "AdditionalInformation" && currentException is BaseApplicationException)
								{
									...
									if (p.GetValue(currentException,null) != null)
									{
										...
										if (currentAdditionalInfo.Count > 0)
										{
                                            ...
                                            strInfoDB.Append("   ****** AdditionalInformation ******");
											...
											for (int i = 0; i < currentAdditionalInfo.Count; i++)
											{
                                                ...
                                                strInfoDB.Append("   [" + currentAdditionalInfo.GetKey(i) + ": " + currentAdditionalInfo[i] + "]");
											}
										}
									}
								}
								...
								else
								{
                                    ...
                                    strInfoDB.Append("   ****** " + p.Name + ": " + p.GetValue(currentException, null) + " ******");
								}
							}
						}
					}
				...
				#region Record the Exception StackTrace
					...
					if (currentException.StackTrace != null)
					{
                        ...
                        strInfoDB.Append(" ****** StackTrace Information ******");
                        strInfoDB.Append("   [" + currentException.StackTrace + "]");
					}
				#endregion
            ...
            WriteToLog(strInfo.ToString(), strInfoDB.ToString(), EventLogEntryType.Error);
        ...
        /// <param name="entryDB">The entry to enter into the database Log.</param>
		/// <param name="type">The EventLogEntryType to be used when the entry is logged to the Event Log.</param>
        private void WriteToLog(string entry, string entryDB, EventLogEntryType type)
		{
            try
            {
                //// Write the entry to the Event Log.
                ...
                parameters[0] = entryDB;
                parameters[1] =Environment.MachineName;
                parameters[2] =1;
                DALBaseClass objDALBaseClass = (new DALBaseClassHelper()).GetDAL();
                objDALBaseClass.InsertData(ref parameters, typeof(DVOExceptionRecord));
            }
        ...
        //        curityException(String.Format(resourceManager.GetString("RES_DEFAULTPUBLISHER_EVENTLOG_DENIED"), applicationName),e);
        //    }
        //}
	}
	#endregion
