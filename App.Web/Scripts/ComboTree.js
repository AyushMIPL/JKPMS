...
; (function ($, window, document, undefined) {
  ...
  ComboTree.prototype.init = function () {
    ...
    this._selectedItem = {};
    this._selectedItems = [];
    this.options.source.forEach((data, index) => {
      if (data.Selected) {
        let selecteditem = { id: data.id, title: data.title };
        this._selectedItems.push(selecteditem);
      }
      if (data.subs.length > 0) {
        data.subs.forEach((sub) => {
          if (sub.Selected) {
            var subselectedItems = { id: sub.id, title: sub.title };
            this._selectedItems.push(subselectedItems);
          }
        });
      }
    });
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    ...
    var itemHtml = "",
      ...
      isSelected = sourceItem.hasOwnProperty("Selected") ? sourceItem.Selected : false,
    ...
    if (isThereSubs)
      itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse || collapse ? '<span class="fa fa-arrow-circle-o-right"></span>' : '<span class="fa fa-arrow-circle-o-down"></span>') + '</span>'; // itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '+' : '&minus;') + '</span>';

    if (this.options.isMultiple) {
      if (isSelected)
        SelectedIDs.push(sourceItem.id);
      itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? (isSelected ? '<input type="checkbox" checked />' : '<input type="checkbox" />') : '') + sourceItem.title + '</span>';
    }
    else
  ...
  ComboTree.prototype.toggleSelectionTree = function (item, direction) {
    ...
    if (direction === undefined) {
      if ($(subMenu).is(':visible'))
        $(item).children('span.comboTreeParentPlus').html('<span class="fa fa-arrow-circle-o-right"></span>'); //$(item).children('span.comboTreeParentPlus').html("+");
      else
        $(item).children('span.comboTreeParentPlus').html('<span class="fa fa-arrow-circle-o-down"></span>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");
